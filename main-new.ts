...
export default class TodosApiPlugin extends Plugin {
	...
	private async processDueDates(app: ObsidianApp, dataviewApi: any, params: {
		...
		// Use metadataCache.fileCache for efficient file discovery
		const fileCache = app.metadataCache.fileCache;
		const allFiles = Object.keys(fileCache);
		

		console.log(`🔍 DEBUG: Found ${allFiles.length} files in metadata cache`);

		// Filter files based on courseId or query parameters
		let candidateFiles = allFiles.filter(filePath => {
			// Skip non-markdown files
			if (!filePath.endsWith('.md')) return false;
			
			// Apply courseId filtering if provided
			if (courseId && !filePath.includes(courseId)) return false;
		...
		console.log(`🔍 DEBUG: After filtering: ${candidateFiles.length} candidate files`);
		...
		for (const filePath of candidateFiles) {
			...
			try {
				...
				if (!file || !(file instanceof TFile)) {
					console.log(`🔍 DEBUG: SKIPPING - File not found or not TFile: ${filePath}`);
					continue;
				}
				...
				if (!matches) {
					console.log(`🔍 DEBUG: No \"Due Dates\" section found in ${filePath}`);
					continue;
				}

				console.log(`🔍 DEBUG: Found Due Dates section in ${filePath}`);
				const tableData = matches[1].trim();
				console.log(`🔍 DEBUG: Table data preview: \"${tableData.substring(0, 100)}\"`);
					
				const lines = tableData.split('\\n').slice(1); // Skip header row
				console.log(`🔍 DEBUG: Found ${lines.length} data lines in table`);

				// Process each table row
				for (const line of lines) {
					console.log(`🔍 DEBUG: Processing table line: \"${line}\"`);
					const columns = line
						.split('|')
						.map(c => c.trim())
						.filter(c => c);
					...
					if (columns.length < 2) {
						console.log(`🔍 DEBUG: SKIPPED - Not enough columns (${columns.length})`);
						continue;
					}

					let [dueDate, assignment] = columns;
					console.log(`🔍 DEBUG: Due date: \"${dueDate}\"`);
					console.log(`🔍 DEBUG: Assignment: \"${assignment}\"`);
						
					// Skip invalid dates and completed items (✅)
					if (!Date.parse(dueDate)) {
						console.log(`🔍 DEBUG: SKIPPED - Invalid date: \"${dueDate}\"`);
						continue;
					}
					if (assignment?.includes('✅')) {
						console.log(`🔍 DEBUG: SKIPPED - Completed assignment: \"${assignment}\"`);
						continue;
					}
					...
					console.log(`🔍 DEBUG: Date filtering - start: ${startDate}, end: ${endDate}`);
					console.log(`🔍 DEBUG: Due date: ${dueDate}, start check: ${dueDateObj.isAfter(startObj)}, end check: ${dueDateObj.isBefore(endObj)}`);
					...
					if (start || end) {
						...
						if (!dueDateObj.isBetween(startObj, endObj, 'day', '[]')) {
							console.log(`🔍 DEBUG: SKIPPED - Date ${dueDate} not in range ${startDate} to ${endDate}`);
							continue;
						}
					} else {
						...
						if (dueDateObj.isBefore(moment().subtract(30, 'days'))) {
							console.log(`🔍 DEBUG: SKIPPED - Date ${dueDate} is too far in the past`);
							continue;
						}
					}
					...
					if (query && !assignment.toLowerCase().includes(query.toLowerCase())) {
						console.log(`🔍 DEBUG: SKIPPED - Assignment doesn't match query \"${query}\"`);
						continue;
					}
					...
					const formattedAssignment = assignment.match(/[A-Z]{3}-[0-9]{3}/)
						? assignment
						: `#${filePath.split('/')[0] || courseId || 'unknown'} - ${assignment}`;
					console.log(`🔍 DEBUG: Formatted assignment: \"${formattedAssignment}\"`);
					...
					console.log(`🔍 DEBUG: ✅ ADDING ENTRY - dueDate: ${dueDate}, assignment: ${formattedAssignment}`);
					// Add the entry
					entries.push({
						dueDate,
						formattedDueDate,
				...
				console.error(`🔍 DEBUG: Error processing file ${filePath}:`, error);
			}
		}

		console.log(`🔍 DEBUG: Total entries found: ${entries.length}`);
		// Sort by due date
